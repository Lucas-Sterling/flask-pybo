...
def modify_question(comment_id):
    ...
    form = CommentForm(obj=comment)  # ★ 기존에는 POST일 때만 있었음 → 수정해서 GET도 커버
...
def modify_answer(comment_id):
    ...
    form = CommentForm(obj=comment)  # GET 요청에서도 form 전달

    if request.method == 'POST' and form.validate_on_submit():
        comment.content = form.content.data   # populate_obj 대신 명시적으로
        comment.modify_date = datetime.now()
        db.session.commit()
        return redirect('{}#comment_{}'.format(url_for(
